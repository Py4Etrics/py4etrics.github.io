...
`singmale`を考えてみよう。引数の`row`は行を表すので、例として`wage1`の0番目の行を考えてみよう。その行には`female`と`married`の例が含まれており、それらの列の情報に基づいて返り値を設定している。もし`female`の値が0であり、さらに`married`の値も0であれば1を返す。そうでなければ0を返す関数となっている。他の関数も同様に考えることができる。

...
実は、この考え方（品物と参照記号の関係）は関数の名前とオブジェクトとしての関数の関係にも当てはまるのである。変数とオブジェクトのこのような関係により、関数がメソッドや関数の引数になることが可能となり、関数の返り値にもなり得るのである。
...
---
ではどのような場合に`views`になるのか。残念ながら、複雑な場合分けが必要になり、それを覚え使いこなすにはある程度の慣れが必要だる。従って、`views`をどうしても使わなくてはならない状況を除いては、以下のようにメソッド`.copy()`を使い、コピーを生成して作業することを強く推奨する。`.copy()`は「深いコピー」を生成する。
...
`views`の役目はなんなのか、という疑問が生じる。`.copy()`は新たにオブジェクトを生成するが、`views`であれば新たなオブジェクトを生成する必要がない。その分、処理速度を早くすることが可能となるのである。しかし、実証分析で行う「通常」のデータ処理であれば`.copy()`を使って問題ないだろう。

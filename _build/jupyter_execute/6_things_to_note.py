...
`.copy()`で生成したコピーは**「浅いコピー」**
（shallow copy）と呼ばれる。しかし、浅いコピーの場合でも、リストが二重配列（入れ子）になっている場合は上で説明した同じ問題が発生する。その場合には、標準ライブラリ`copy`の`deepcopy`関数を使い独立したコピーを作ることが可能となる。これを**「深いコピー」**
（deep copy）呼ぶ。
